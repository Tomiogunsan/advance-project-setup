...
isError, 
isPreviousData,
} = useQuery<QuotesData>(['quotes', page], () => fetchQuotesByPage(page), { keepPreviousData: true,
})
...
<button
onClick={() => setPage((old) => Math.max(old - 1, 0))} className={page === 1 ? 'text-gray-400' : ''} disabled={page === 1}
>
Previous
</button>{' '}
<span className="text-lg font-italic">{page}</span> <button
onClick={() => {
...
{isFetching ? <span>Loading...</span> : null}{' '} </div>
) : null} </div>
</div>
) }
