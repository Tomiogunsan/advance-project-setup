...
const {
data: quotes, isLoading, isFetchingNextPage, isSuccess, isError, fetchNextPage, hasNextPage,
} = useInfiniteQuery<QuotesDataWithCursor>(
'quotes',
({ pageParam = 0 }) => fetchQuotesByCursor(pageParam), {
getNextPageParam: (lastPage, pages) => { return lastPage.nextCursor
}, }
)
useEffect(() => {
...
return (
    <div className="py-8 max-w-2xl mx-auto"> <div>
...
{isSuccess ? ( <div>
<div className="max-h-96 overflow-y-auto divide-y"> {quotes?.pages.map((data) => {
return data.quotes.map((quote) => { return (
<blockquote
key={quote.id}
...
<div ref={loadMoreRef}></div> </div>
{isFetchingNextPage ? <span> Loading...</span> : null}{' '} </div>
) : null} </div>
</div>
) }
